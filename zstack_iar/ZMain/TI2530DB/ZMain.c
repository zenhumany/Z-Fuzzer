...
{
  // Turn off interrupts
  // #define INST_ALL 0xFF -> Call EA=0;
  // Hardware - OSAL.c
  ...
  // Hardware - could manually return true for experiment
  zmain_vdd_check();

  // Initialize board I/O
  // OnBoard.c - need user update
  InitBoard( OB_COLD );

  // Initialze HAL drivers, ADC, DMA, AES, LED, LCD, KEY
  // UART AND SPI only have declaration, no implementation
  // Hardware
  HalDriverInit();

  // Initialize NV System
  // Hardware NVRAM - OSAL_Nv.c
  osal_nv_init( NULL );
  ...
  // Determine the extended address, IEEE 64 bits address
  // Related to NVRAM
  zmain_ext_addr();

#if defined ZCL_KEY_ESTABLISH   // No definition
  // Initialize the Certicom certificate information.
  zmain_cert_init();
#endif

  // Initialize basic NV items
  // Not exist - use default. Value are set in OSAL/ZComDef.h
  zgInit();

#ifndef NONWK
  // Since the AF isn't a task, call it's initialization routine
  afInit();        // Nothing to do.
#endif
...
#ifdef LCD_SUPPORTED  // No definition
  zmain_lcd_init();
#endif

#ifdef WDT_IN_PM1    // No definition
