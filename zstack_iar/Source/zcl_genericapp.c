...
endPointDesc_t *GenericApp_epDesc;   //for experiment_1

uint8 zclGenericApp_TransID = 0;
...
static void GenericApp_SendMessage(void);
...
void zclGenericApp_Init( byte task_id )
{
  ...
  GenericApp_epDesc = osal_mem_alloc( sizeof ( endPointDesc_t ) );
  if ( GenericApp_epDesc )
  {
    // Fill out the endpoint description.
    GenericApp_epDesc->endPoint = GENERICAPP_ENDPOINT;
    GenericApp_epDesc->task_id = &zclGenericApp_TaskID;   // all messages get sent to ZCL first
    GenericApp_epDesc->simpleDesc = &zclGenericApp_SimpleDesc;
...
uint16 zclGenericApp_event_loop( uint8 task_id, uint16 events )
{
  ...
  if ( events & GENERICAPP_EVT_1 )
  {
    ...
    if ( zclGenericApp_NwkState == DEV_ZB_COORD )
    {
      ...
      osal_start_timerEx(zclGenericApp_TaskID, GENERICAPP_EVT_1, 2000);
    }
  }
...
{
  char buffer[25];
  if ( zclGenericApp_NwkState == DEV_ZB_COORD ) // Coordinator receives response
  {
    switch(pkt->clusterId)
    {
      case ZCL_BASIC:
        osal_stop_timerEx(zclGenericApp_TaskID, GENERICAPP_EVT_1);
        osal_memcpy(buffer,pkt->cmd.Data,25);
        HalLcdWriteString( (char *)buffer, HAL_LCD_LINE_4 ); // Print out the received message
        break;
    }
  }
  if ( zclGenericApp_NwkState == DEV_END_DEVICE )
  {
    char expMsg[25] = "Coordinator send!";
    switch(pkt->clusterId)
    {
      case ZCL_BASIC:
        osal_memcpy(buffer,pkt->cmd.Data,osal_strlen((char *)expMsg+1));
        if(osal_memcmp(buffer,expMsg,osal_strlen((char *)expMsg+1)))
        {
          HalLedSet ( HAL_LED_2, HAL_LED_MODE_TOGGLE );
        }
        GenericApp_SendMessage();
        HalLcdWriteString( (char *)buffer, HAL_LCD_LINE_5 );  // Print out the received message
        break;
    }
  }
}

void GenericApp_SendMessage(void)
{
  if (zclGenericApp_NwkState == DEV_ZB_COORD)
  {
    char *msg = "Coordinator send!";
    afAddrType_t my_DstAddr;
    my_DstAddr.addrMode = (afAddrMode_t)AddrBroadcast;
    my_DstAddr.endPoint = GENERICAPP_ENDPOINT;
    my_DstAddr.addr.shortAddr = 0xFFFF;

    ZStatus_t status = AF_DataRequest(&my_DstAddr,GenericApp_epDesc,ZCL_BASIC,osal_strlen(msg)+1,msg,&zclGenericApp_TransID,AF_DISCV_ROUTE,AF_DEFAULT_RADIUS);
    if( status == ZSuccess )
    {
      HalLcdWriteString( (char *)msg, HAL_LCD_LINE_4 ); // Print out the sending message
    }
  }
  if (zclGenericApp_NwkState == DEV_END_DEVICE)
  {
    char *msg = "EndDevice received!";
    afAddrType_t my_DstAddr;
  ...
  switch(bdbCommissioningModeMsg->bdbCommissioningMode)
  {
    case BDB_COMMISSIONING_FORMATION:
      if(bdbCommissioningModeMsg->bdbCommissioningStatus == BDB_COMMISSIONING_SUCCESS)
      {
        ...
        HalLcdWriteString( "Formation done.", HAL_LCD_LINE_5 );
      }
      else
      {
