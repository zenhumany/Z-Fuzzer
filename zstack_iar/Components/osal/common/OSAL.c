...
uint8 osal_init_system( void )
{
  ...
  // osal_msg_q_t osal_qHead;
  // Void pointer, could point to any type
  osal_qHead = NULL;
...
{
#ifdef USE_ICALL  // Used on CC2630
...
void osal_run_system( void )
{
  ...
  Hal_ProcessPoll();   // Poll UART and Timer
#endif /* USE_ICALL */
  ...
  if (idx < tasksCnt)     // run task
  {
    uint16 events;
    halIntState_t intState;

    HAL_ENTER_CRITICAL_SECTION(intState);    // ½øÈëÁÙ½çÇø int_disable
    events = tasksEvents[idx];               // È¡³öÊÂ¼þ
    tasksEvents[idx] = 0;  // Clear the Events for this task. 
    HAL_EXIT_CRITICAL_SECTION(intState);     // ÍË³öÁÙ½çÇø int_enable

    activeTaskID = idx;
    events = (tasksArr[idx])( idx, events );  // Call task function by pointer
    activeTaskID = TASK_NO_TASK;

    HAL_ENTER_CRITICAL_SECTION(intState);     //½øÈëÁÙ½çÇø±£´æÎ´´¦ÀíÊÂ¼þ
